"""
shark_tank_us_miner.py — Autonomous Job Sourcing & Decision Maker Outreach Engine
for Shark Tank US Startups (Seasons 1-16).

Features:
1. Sourcing live engineering roles from US Shark Tank company career boards.
2. Mining Founders, Co-Founders, and CTO decision makers.
3. Strict MAX 2 Decision Makers per Company limit.
4. Shark Tank US-Tailored Cold Outreach Composer & SMTP Delivery.
"""
...
from src.shark_tank_us_startups import SHARK_TANK_US_REGISTRY, SharkTankUSStartup

logger = logging.getLogger("shark_tank_us_miner")
...
class SharkTankUSMiner:
    """Autonomous miner for Shark Tank US startups, leadership, and engineering openings."""
    ...
    async def mine_shark_tank_us_decision_makers(
        self, startup: SharkTankUSStartup, max_results: int = 6
    ) -> List[Dict[str, Any]]:
        """Search and extract founders, CTOs, and technical leaders for Shark Tank US startup."""
        ...
        try:
            ...
            logger.warning(f"Error mining leaders for US Shark Tank startup {startup.name}: {exc}")

        return contacts

    def compose_shark_tank_us_outreach(self, contact: Dict[str, Any]) -> Tuple[str, str, str]:
        """Compose high-converting cold email tailored specifically to Shark Tank US alumni."""
        ...
        subject = f"Engineering & Scale at {company} (Shark Tank Season {season}) — Kushall Jain"

        body_text = f"""Hi {first_name},

I've been following {company}'s massive growth journey since your appearance on Shark Tank (Season {season}) and backing from {sharks}.

I'm a Software Engineer specializing in Python, FastAPI, React, and high-performance distributed systems. Scaling a breakout brand post-Shark Tank requires bulletproof API infrastructure, real-time data pipelines, and seamless user experiences under heavy demand.

Given your leadership as {title}, I wanted to reach out directly to see if you're looking for a proactive, high-velocity engineer to help scale your digital architecture and deliver core product capabilities.

Key Highlights:
• Full-stack development with FastAPI, Python, React & Next.js
• High-throughput backends and event-driven data systems (PostgreSQL, Redis, Kafka)
• Ownership-driven mindset: fast execution, clean code, and production reliability

My LinkedIn: {self.sender_linkedin}

Would you be open to a brief 10-minute call this week to see if my engineering background could add value to {company}?
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; color: #1E293B; line-height: 1.6;">
  <p>Hi {first_name},</p>
  <p>I've been following <strong>{company}</strong>'s massive growth journey since your appearance on <strong>Shark Tank (Season {season})</strong> and backing from {sharks}.</p>
  <p>I'm a Software Engineer specializing in <strong>Python, FastAPI, React</strong>, and high-performance distributed systems. Scaling a breakout brand post-Shark Tank requires bulletproof API infrastructure, real-time data pipelines, and seamless user experiences under heavy demand.</p>
  <p>Given your leadership as <em>{title}</em>, I wanted to reach out directly to see if you're looking for a proactive, high-velocity engineer to help scale your digital architecture and deliver core product capabilities.</p>
  <p><strong>Key Highlights:</strong></p>
  <ul>
    <li>Full-stack development with FastAPI, Python, React & Next.js</li>
    <li>High-throughput backends and event-driven data systems (PostgreSQL, Redis, Kafka)</li>
    <li>Ownership-driven mindset: fast execution, clean code, and production reliability</li>
  </ul>
  <p>Would you be open to a brief 10-minute call this week to see if my engineering background could add value to {company}?</p>
  <p>Best regards,<br>
  ...
  <a href="{self.sender_linkedin}" style="color: #4F46E5;">{self.sender_linkedin}</a>
    ...
    async def mine_and_outreach_startup(
        self, startup: SharkTankUSStartup, auto_send: bool = False
    ) -> List[Dict[str, Any]]:

        """Mine decision makers for single Shark Tank US startup and dispatch outreach with strict <= 2 cap."""
        from sqlalchemy import func

        contacts = await self.mine_shark_tank_us_decision_makers(startup, max_results=6)
        if not contacts:
            return []
            ...
            for dm in contacts:
                ...
                if not existing:
                    new_c = Contact(
                        ...
                        confidence_score=90,
                        source=f"shark_tank_us_s{startup.season}",
                ...
                if auto_send and slots_remaining > 0:
                    ...
                    if not already_sent and dm["email"]:
                        subj, text, html = self.compose_shark_tank_us_outreach(dm)
                        loop = asyncio.get_event_loop()
                        ...
                        if sent_ok:
                            ...
                            rec = OutreachRecord(
                                ...
                                subject=subj,
                                body=text,
                                template_type="shark_tank_us_founder_outreach",
                                status="sent",
                                email_sent=True,
                            ...
                            db.add(rec)
                            db.commit()
                            logger.info(f"Sent Shark Tank US outreach to {dm['name']} ({dm['title']} at {dm['company']}) -> {dm['email']}")
...
shark_tank_us_miner = SharkTankUSMiner()
